                        ...
                        <div class="col-lg-4 col-md-6">
                                    ...
                                    <div class="zum_icvoice_item">
                                        <ul class="zum_invoice_lixt">
                                            <li>{{__('translate.Full Name')}} :
                                                <span>{{$address?->contact_person_name ?? $order->user?->name ?? ''}}</span></li>
                                            <li>
                                                <a href="mailto:{{$address?->contact_person_email ?? $order->user?->email ?? ''}} ">
                                                    {{__('translate.Email')}} :
                                                    <span> {{$address?->contact_person_email ?? $order->user?->email ?? ''}} </span>
                                                </a>
                                            </li>
                                            <li>
                                                <a href="tel:{{$address?->contact_person_number ?? $order->user?->phone ?? ''}}">
                                                    {{__('translate.Phone')}} :
                                                    <span> {{$address?->contact_person_number ?? $order->user?->phone ?? ''}}</span>
                                                </a>
                                            </li>
                                            <li>
                            ...
                            @if($order->order_type == 'pickup')
                                <p><strong>{{__('translate.Contact person name')}}
                                        : </strong> {{$address?->contact_person_name ?? $order->user?->name ?? ''}}</p>
                                <p><strong>{{__('translate.Contact person phone')}}
                                        : </strong> {{$address?->contact_person_number ?? $order->user?->phone ?? ''}}</p>
                                <p><strong>{{__('translate.Contact person email')}}
                                        : </strong> {{$address?->contact_person_email ?? $order->user?->email ?? ''}}</p>
                            @endif
                        </div>
                        <div class="col-lg-4 col-md-6">
                            <div class="zum_icvoice_item_main">
                            ...
                            <div class="zum_icvoice_item_main">
                                <div class="d-flex align-items-center justify-content-between flex-wrap gap-2 mb-3">
                                    <div class="d-flex align-items-center gap-2">
                                        ...
                                        <span class="badge" id="restRouteStatsBadge" style="background: #e0f2fe; color: #0284c7; font-weight: 600; font-size: 12px; padding: 5px 10px; border-radius: 6px;">
                                ...
                                <div id="restaurant_order_details_map" style="height: 300px; width: 100%; border-radius: 12px; border: 1.5px solid #cbd5e1; z-index: 1;"></div>
                            </div>
                        </div>
                    </div>
                                ...
                                <table class=" zum_tabel table table-striped">
                                    ...
                                    @foreach($order->items ?? [] as $key => $order_item)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if($product)
                                                    <a href="{{ route('restaurant.product.edit', ['product' => $product->id, 'lang_code' => admin_lang()] ) }}">{{$product->name}}</a>
                                                @else
...
@push('js_section')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            const mapEl = document.getElementById('restaurant_order_details_map');
            if (!mapEl) return;
            ...
            const map = L.map('restaurant_order_details_map', {
            ...
            if (origLat !== 0 && origLng !== 0) {
                ...
                restMarker.bindPopup(`<b>🏪 {{ addslashes($order->restaurant?->restaurant_name ?? __('translate.Your Restaurant')) }}</b><br><small>{{ addslashes($order->restaurant?->address ?? '') }}</small>`).openPopup();
                markers.push(restMarker);
            }

            if (destLat !== 0 && destLng !== 0) {
                const destMarker = L.marker([destLat, destLng], { icon: destPin }).addTo(map);
                destMarker.bindPopup(`<b>📍 {{ __('translate.Delivery Destination') }}</b><br><small>{{ addslashes($order?->address?->address ?? ($addressObj?->address ?? '')) }}</small>`);
                markers.push(destMarker);
            }

            const statsBadge = document.getElementById('restRouteStatsBadge');
