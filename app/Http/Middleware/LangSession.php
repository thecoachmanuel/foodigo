...
class LangSession
{
    ...
    {
       // Set language session if not already set
        if (!Session::has('front_lang')) {
            $default_lang = \Illuminate\Support\Facades\Cache::remember('default_lang', 3600, function() {
        ...
        // Set currency session if not already set
        if (!Session::has('currency_code')) {
            $activeCurrency = \Illuminate\Support\Facades\Cache::remember('default_active_currency', 3600, function() {
                return Currency::where('is_default', 'yes')->where('status', 'active')->first()
